...
class Matrix:
    def __init__(self, app, font_size=8):
        ...
        self.katakana = np.array([chr(int(0) + i) for i in range(5)] + ['' for i in range(10)])
        # self.katakana = np.array([chr(int('0x30a0', 16) + i) for i in range(96)] + ['' for i in range(10)])
        self.font = pg.font.Font('resourses/ms mincho.ttf', font_size)
        ...
        self.image = self.get_image('resourses/cat.jpg')
...
class MatrixVision:
    ...
    def draw(self):
        ...
        self.matrix.run()
        # Draw surface it onto the screen and position it at (x,y)
